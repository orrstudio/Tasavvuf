...
- **Технические компоненты**:  
  ...
  - `pages/` — подпапки для каждого раздела меню; в каждой подпапке – HTML‑файлы для каждой кнопки.  
  - `pages/info/about.html` — страница с информацией из фута.  
  - `CSS/Main.css` — все стили, включая медиазапросы для адаптивности.  
  - `fonts/` — локальные шрифты.  
  - `icons/` — SVG‑иконки: `burger.svg` (гамбургер).  
...
- **Архитектура**:  
  ...
  - Страница `pages/info/about.html` содержит информацию из фута.  
- **Правила оформления**:  
...
> Гамбургер‑иконка – локальный SVG‑файл (`icons/burger.svg`), визуально совпадающий с текущей кнопкой меню.
...
### 5. Создать страницу `pages/info/about.html`
...
> **Содержимое about.html** – скопировать информацию из фута текущего `index.html` (если есть) или создать новую страницу с информацией о сайте, контактами и т.д.  
...
- Убедиться, что:  
  ...
  5. Скрипты (например, `date.js`) подгружаются **только там, где они действительно нужны** (в `about.html` и в тех разделах, где требуется).  
  ...
  8. Все названия файлов и папок на английском.  
...
- **Уникальные URL** – каждая страница (`pages/.../*.html`) имеет своё адресное пространство и может быть разделена/поделилась.  
- **Внешние ссылки** (`https://mihr.com/...`) сохраняют свои атрибуты `target="_blank" rel="noopener noreferrer"` и открываются в новом окне.  
- **Шрифты/иконки** находятся в локальных папках (`fonts/`, `icons/`) и подключаются через `<link>` и `<img>` в HTML.  
